...
Deep Convolutional Neural Networks (CNNs), starting with AlexNet and formalized for detection in R-CNN, improved feature representation. Two-stage detectors divide object detection into two steps:
\begin{enumerate}[(i)]
...
Later architectures, including modern anchor-free models (such as YOLOv8 and YOLO26s), added Path Aggregation Networks (PANet), Cross-Stage Partial (CSP) connections, and Task-Aligned Assigners \cite{jiang2022}. These features fuse shallow high-resolution layers with deep semantic layers.

However, standard detectors trained on benchmarks like MS COCO group all humans into one ``person'' class. When fine-tuned on pediatric datasets such as the Child Detection Dataset (CDD) \cite{diop2025}, these models struggle with close physical proximity. They often merge an infant carried on a mother's back into the adult's bounding box.

\section{Pediatric Anthropometry and Developmental Allometry}
To separate children from adults without relying on variable clothing patterns, computer vision can use biological growth principles from physical anthropology.
...
\begin{equation}
    y = b \cdot x^{\alpha} \quad \iff \quad \ln(y) = \ln(b) + \alpha \ln(x)
\end{equation}
...
\end{equation}
As Bogin \cite{bogin1999} and Mlinari\'{c} et al.\ \cite{mlinaric2024} show, pediatric subjects have $R_{head} \approx 0.25$ and $R_{ceph} \ge 0.85$. Adults have $R_{head} \approx 0.125$ and $R_{ceph} \le 0.70$. Adding these biological ratios directly into detection provides a clear mathematical rule to separate adults and children.

\section{Occlusion Handling in Crowded Clinical Healthcare Environments}
Physical occlusion is a major obstacle in visual monitoring. In outpatient clinics, patients sit close together on wooden benches or crowd around triage desks and pharmacy windows \cite{osei2024}.

\subsection{Taxonomy of Occlusion}
In hospital surveillance, occlusions fall into two groups:
\begin{enumerate}[(i)]
    \item \textbf{Static Environmental Occlusion:} Physical fixtures, including desks, registration counters, pillars, and carts, block patient torsos and legs.
    \item \textbf{Dynamic Inter-Object Occlusion:} Walking or seated patients block each other. In Ghana, mothers carry infants wrapped against their bodies. This causes up to $80\%$ visual occlusion of the child \cite{addotey2023, ouyang2015}.
\end{enumerate}
...
Single-frame detection alone cannot maintain an accurate patient count. A tracking system must connect detections across video frames to prevent counting the same person twice as patients move through corridors.
...
Bewley et al.\ \cite{bewley2016sort} created Simple Online and Realtime Tracking (SORT). SORT uses a linear constant-velocity Kalman filter to predict positions and uses the Hungarian algorithm to match boxes by IoU distance. SORT is fast (over 200 FPS), but it drops lost tracks after only a few occluded frames.
...
Deploying vision models in developing health facilities introduces strict infrastructure constraints: unstable power, no dedicated GPU servers, and privacy rules that prevent sending raw video to cloud servers \cite{milkaite2019, addotey2023}.
...
To run models in real time on hospital CPUs without GPUs, modern systems use the Open Neural Network Exchange (ONNX) Runtime and Single Instruction, Multiple Data (SIMD) instructions.
...
\begin{enumerate}[(i)]
    \item \textbf{The Semantic-Biological Disconnect:} Existing deep learning detectors optimize bounding boxes using geometric criteria (IoU, aspect ratio) alone. No prior work integrates allometric cephalocaudal growth laws ($H/B$ and $L_{torso}/L_{leg}$) directly as a verification rule for pediatric patient counting.
    \item \textbf{The Occlusion Feature-Collapse Gap:} Standard models run on downsampled full frames, which removes the fine visual details of carried infants. Slicing-aided inference (SAHI) and contrast enhancement (CLAHE) have not been combined for pediatric clinical triage.
