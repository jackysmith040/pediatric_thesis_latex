...
This chapter presents a multi-stage, privacy-preserving computer vision framework to count pediatric patients in hospital triage environments under heavy occlusion and variable lighting. The architecture integrates full-frame detection, secondary crop classification, and biological verification, as illustrated in Figure \ref{fig:system_arch}:
\begin{enumerate}[(i)]
...
\begin{figure}[htbp]
    ...
    \caption{Multi-Stage Pediatric Detection, Tracking, and Census Architecture.}
...
Figure \ref{fig:quant_matrix} shows a digital patch $I(x, y)$ along a patient edge. In a $5 \times 5$ sub-patch, the values change from dark background tiles ($I \approx 10-15$) to child clothing ($I \approx 95-105$), producing a step edge of $+84$ intensity units between column 2 and column 3:
\begin{equation}
...
\end{enumerate}
These distillation errors led us to pivot from soft-logit distillation to a dedicated dual-stage architecture with explicit biological constraints.
...
\section{Stage 3: Bayesian Prior and Multi-Modal Consensus Fusion}
Single-frame classifier scores fluctuate under camera noise, perspective tilt, and motion blur. Stage 3 stabilizes classifications by fusing physical priors and temporal consensus.

\subsection{Bayesian Height Prior and Stature Modulation}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
where $\theta_{\text{adult}} = 0.32$, calibrated against empirical clinical standing measurements (adults: $0.334\text{--}0.420$, children: $0.170\text{--}0.274$).
...
\begin{figure}[htbp]
    ...
    \caption{Temporal Bayesian Tracklet Belief Update: Posterior Probability Convergence Over Frames.}
...
In hospital waiting halls, seated patients often trigger two fragmented bounding boxes: one for the upper torso/head and one for the lap/knees. Standard IoU fails to merge them because horizontal overlap is minimal. The system detects anatomical fragmentation when:
\begin{enumerate}[(i)]
...
To track patients across multi-room clinics without cloud dependencies, the architecture includes a decoupled Person Re-Identification plugin.
