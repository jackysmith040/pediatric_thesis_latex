...
The distilled student model had low accuracy ($F_1 = 0.736$, $\text{MAPE} = 26.2\%$). Because the teacher network was trained on noisy labels where backpacks and cloth swaddles were labeled as children, distillation transferred these false labels to the student. In contrast, our multi-stage pipeline---combining YOLO26s with margin-expanded crop classification, allometric pose verification, and weighted temporal smoothing---achieved an $F_1$-score of $0.988$ and reduced headcount MAPE to $0.0\%$ on the evaluation test sequences.
...
\begin{itemize}
    \item \textbf{Curated Sequence Evaluation:} Across the seven standardized test sequences (700 total frames with 46 verified individuals), the multi-stage pipeline matched ground-truth headcount after applying weighted temporal smoothing.
